...
# ==========================================
# 2. 공통 백엔드 연산 엔진 (yfinance 순수 연산 자립화)
# ==========================================
def analyze_stock_score(ticker_code, stock_name):
    ...
    # 💡 pykrx 완전 제거에 따른 yfinance 자산 가치 지표 전환 파싱
    per, pbr = 0.0, 0.0
    try:
    ...
    try:
        ...
        for item in root.findall('.//item')[:10]:
            ...
            if title and title not in seen_titles:
                ...
                news_list.append({"title": f"{title} ({time_str})", "link": link, "pub_dt": pub_dt})
    except: pass
    return news_list[:6]
    ...
    if not ticker_code:
        ...
        if not res_data:
            ...
            if last_close > last_open:
                candle_type = "🔴 양봉"
                if candle_body > (last_open * 0.04): candle_desc = "장대양봉이 출현하며 바닥권에서 강력한 매수세 유입을 증명하고 있습니다."
                elif candle_lower_tail > candle_body * 2: candle_desc = "아래꼬리가 긴 망치형 양봉입니다. 저가 매수세가 하락을 완벽히 방어했습니다."
                else: candle_desc = "일반적인 상승형 양봉입니다. 단기 매수 우위 상태를 나타냅니다."
            else:
                candle_type = "🔵 음봉"
                if candle_body > (last_open * 0.04): candle_desc = "장대음봉이 출현하며 단기 차익 실현 물량 및 강한 매도 압력이 지배하고 있습니다."
            ...
            if candle_body < (last_open * 0.005):
                ...
                candle_desc = "시가와 종가가 거의 일치하는 십자형 도지 캔들입니다. 매수세와 매도세가 팽팽히 맞서며 강력한 추세 전환(변곡점) 임박을 예고합니다."
            ...
            if final_score >= 75:
                decision_text, opinion, strategy_text = "🔥 강력 매수", "🔥 강력 매수", "안전마진과 차트 변곡점이 모두 융합된 최적의 바닥 타점입니다."
            elif final_score >= 50:
                decision_text, opinion, strategy_text = "✅ 분할 매수", "✅ 분할 매수", "하단 지지선을 디딤돌 삼아 장기 물량을 모아가기 좋은 구간입니다."
            else:
                decision_text, opinion, strategy_text = "🚨 매수 금지", "🚨 매수 금지", "악재 수렴 중이거나 차트가 고점 과열 상태입니다. 관망하십시오."
            ...
            with left_col:
                ...
                with tab1:
                    ...
                    st.markdown(f"**🔹 거시경제 분석가:** 금리 환경과 글로벌 매크로 유동성을 대조했을 때, {stock_name}의 현 주가는 하방 경직성을 확보한 방어적 매력이 돋보이는 위치입니다.")
                    st.markdown(f"**🔹 기본적 분석가:** 내부 파싱 밸류에이션(PER {res_data['per']:.2f}배, PBR {res_data['pbr']:.2f}배) 연산 결과, 자산 가치 대비 확실한 안전마진이 확보된 저평가 매수 영역으로 판독됩니다.")
                    st.markdown(f"**🔹 기술적 분석가:** 캔들 몸통 대비 꼬리 비율 추적 결과 단기 저점 지지 강도가 높으며, RSI가 {res_data['rsi']:.1f}점으로 과매도 구간을 탈출하려는 에너지가 누적되고 있습니다.")
                    st.markdown(f"**🔹 수급 분석가:** 포털 거래 대금 상위 데이터 매칭 결과 대량 대기 매수세가 유입되는 변곡 거래량이 포착되었습니다.")
                    st.markdown(f"**🔹 리스크 관리자:** AI 뉴스 공시 데이터 감성 분석 결과 돌발성 상장폐지나 펀더멘탈 훼손 악재는 발견되지 않아 원칙적 진입이 허용됩니다.")
                with tab2:
                ...
                with tab3:
                    for news in advanced_news: st.markdown(f"- ⚪ | [{news['title']}]({news['link']})")
            with right_col:
